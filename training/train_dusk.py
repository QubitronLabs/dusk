...
from models import MAGVITv2, get_mask_schedule, DuskModelLM, DuskConfig
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(config.model.dusk.pretrained_model_path, padding_side="left")
    ...
    # Initialize dusk in pretraining stage 
    base_config = AutoConfig.from_pretrained(config.model.dusk.pretrained_model_path).to_dict()
    dusk_config_dict = {k: v for k, v in config.model.dusk.items()}
    merged_config = {**base_config, **dusk_config_dict}
    dusk_config = DuskConfig(**merged_config)
    model = DuskModelLM.from_pretrained(config.model.dusk.pretrained_model_path, torch_dtype=torch.bfloat16, config=dusk_config)
    model.resize_token_embeddings(dusk_config.new_vocab_size)
...
def visualize_predictions(
    ...
    predictions = logits[:config.training.batch_size_t2i, -(config.model.dusk.num_vq_tokens + 1):-1:, len(uni_prompting.text_tokenizer) + config.model.dusk.num_new_special_tokens: len(uni_prompting.text_tokenizer) + config.model.dusk.num_new_special_tokens + config.model.dusk.codebook_size]
    
    predictions = predictions.argmax(axis=-1)
    mask_token_id = accelerator.unwrap_model(model).config.mask_token_id - len(uni_prompting.text_tokenizer)
    input_ids = input_ids[:config.training.batch_size_t2i, -(config.model.dusk.num_vq_tokens + 1):-1:] - len(uni_prompting.text_tokenizer)
    mask_ratio = list((torch.where(input_ids == mask_token_id, 1, 0).sum(
        dim=-1) / config.model.dusk.num_vq_tokens).cpu().numpy())
...
def generate_images(
    ...
    image_tokens = torch.ones((len(validation_prompts), config.model.dusk.num_vq_tokens), dtype=torch.long,
    ...
    with torch.autocast("cuda", dtype=weight_dtype, enabled=accelerator.mixed_precision != "no"):
        ...
        gen_token_ids = accelerator.unwrap_model(model).t2i_generate(
            ...
            seq_len=config.model.dusk.num_vq_tokens,
