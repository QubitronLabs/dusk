...
def main():
    device = 'cuda'
    model = DuskModelLM.from_pretrained("QubitronLabs/dusk-8b-mixcot", trust_remote_code=True, torch_dtype=torch.bfloat16).to(device).eval()
    tokenizer = AutoTokenizer.from_pretrained("QubitronLabs/dusk-8b-mixcot", trust_remote_code=True)
