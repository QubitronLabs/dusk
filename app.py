...
from models import MAGVITv2, get_mask_schedule, DuskModelLM
...
DEFAULT_MODEL_PATH = "QubitronLabs/dusk-8b" # Default
CURRENT_MODEL_PATH = None

MODEL_CHOICES = [
    "Dusk-8B",
    "Dusk-8B-CoT (coming soon)",
    "Dusk-8B-Max (coming soon)"
]
MODEL_ACTUAL_PATHS = {
    "Dusk-8B": DEFAULT_MODEL_PATH, 
}
...
def _load_model_and_tokenizer_core(model_path_to_load, model_display_name_for_status):
    ...
    try:
        ...
        MODEL = DuskModelLM.from_pretrained(model_path_to_load, trust_remote_code=True, torch_dtype=torch.bfloat16).to(DEVICE).eval()
...
# with gr.Blocks() as demo:
    ...
    gr.Markdown("<h1 style='text-align: center; margin-bottom: 20px;'>Dusk: Diffusion Language Model</h1>")
    gr.Markdown("Dusk is a diffusion-based language model designed for high-quality text generation using iterative denoising")
    gr.Markdown("Github: [QubitronLabs/dusk](https://github.com/QubitronLabs/dusk)")
    gr.Markdown("Paper: [Dusk: Diffusion Language Model]()")
...
if __name__ == "__main__":
    ...
    demo.launch(share=True, css=css_styles, theme=theme)