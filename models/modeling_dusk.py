...
from .modeling_dusk_base import DuskBaseLM
...
def get_num_transfer_tokens(mask_index, steps):
    ...
    Furthermore, because Dusk employs a linear noise schedule (as defined in Eq. (8)),
...
class DuskConfig(PretrainedConfig):
    model_type = "dusk"
    ...
    def __init__(self, **kwargs):
        ...
        # Defaults for keys needed by forward()
...
class DuskModelLM(DuskBaseLM):
    config_class = DuskConfig
    base_model_prefix = "model"
    def __init__(self, config: DuskConfig, *args, **kwargs):
        print(f"Initializing DuskModelLM with config: {config}")
    ...
    def t2i_generate(
        ...
        # print(f"config.model.dusk.llm_vocab_size: {config.model.dusk.llm_vocab_size}, {len(uni_prompting.text_tokenizer)}")
    ...
    def mmu_generate(self, idx=None, input_embeddings=None, max_new_tokens=128, steps=128,block_length=128, temperature=0.0, top_k=None, eot_token=None, cfg_scale=0.0, remasking='low_confidence', mask_id=126336, attention_mask=None):
        ...
        for num_block in range(num_blocks):
            ...
            #     idx_next_embeddings = self.dusk.model.embed_tokens(idx_next)
...
AutoConfig.register("dusk", DuskConfig)
AutoModelForCausalLM.register(DuskConfig, DuskModelLM)
AutoModel.register(DuskConfig, DuskModelLM)
