...
from .configuration_dusk import (
    DuskBaseConfig,
    StrEnum,
    InitFnType,
    ActivationType,
...
class DuskBlock(nn.Module):
    ...
    @classmethod
    def build(cls, layer_id: int, config: ModelConfig, cache: BufferCache) -> DuskBlock:
        if config.block_type == BlockType.sequential:
            return DuskSequentialBlock(layer_id, config, cache)
        elif config.block_type == BlockType.llama:
            return DuskLlamaBlock(layer_id, config, cache)
        else:
            raise NotImplementedError(f"Unknown block type: '{config.block_type}'")


class DuskSequentialBlock(DuskBlock):
    """
...
class DuskLlamaBlock(DuskBlock):
    """
    This is a transformer block where the output is computed as ``MLP(LN(x + Attention(LN(x))))``
    (plus another skip connection). This block is similar to `DuskSequentialBlock`
...
class DuskGenerateOutput(NamedTuple):
...
class DuskBlockGroup(nn.ModuleList):
...
class DuskTransformer(nn.Module):
    def __init__(self, config: ModelConfig, init_params: bool = True):
        ...
        blocks = [DuskBlock.build(i, config, self.__cache) for i in range(config.n_layers)]
        if self.config.block_group_size > 1:
            block_groups = [
                DuskBlockGroup(config, i, blocks[i : i + config.block_group_size])
    ...
    def forward(
        ...
        return DuskOutput(logits=logits, attn_key_values=attn_key_values, hidden_states=tuple(all_hidden_states) if output_hidden_states else None)  # type: ignore[arg-type]


def create_model_config_from_pretrained_config(config: DuskBaseConfig):
    """
    Utility function
    """
...
class DuskBaseLM(PreTrainedModel):
    """
    Extremely barebones HF model wrapper.
    """

    config_class = DuskBaseConfig
    base_model_prefix = "model"
    _no_split_modules = ["DuskBlock", "DuskSequentialBlock", "DuskLlamaBlock"]
    _tied_weights_keys = []
    ...
    def __init__(self, config: DuskBaseConfig, model: Optional[DuskTransformer] = None, init_params: bool = False):
        ...
        if not model:
            ...
            self.model = DuskTransformer(model_config, init_params=init_params)
        else:
            self.model = model
    ...
    def forward(
        ...
        if output_attentions:
            raise ValueError("output_attentions is not yet supported in Dusk")
        ...
        if labels is not None:
            import warnings
            warnings.warn("Note that for Dusk, you cannot calculate the loss here.", UserWarning)
        if not return_dict:
...
AutoModel.register(DuskBaseConfig, DuskBaseLM)