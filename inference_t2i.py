...
if __name__ == '__main__':
    ...
    vq_model.eval()
    model = DuskModelLM.from_pretrained(config.model.dusk.pretrained_model_path, trust_remote_code=True, torch_dtype=torch.bfloat16)
    ...
    for step in tqdm(range(0, len(validation_prompts), config.training.batch_size)):
        ...
        image_tokens = torch.ones((len(prompts), config.model.dusk.num_vq_tokens),
        ...
        gen_token_ids = torch.clamp(gen_token_ids, max=config.model.dusk.codebook_size - 1, min=0)
